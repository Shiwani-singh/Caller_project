...
class AdminController {
  ...
  async showDashboard(req, res) {
    try {
      ...
      // Defensive: If callerStats is empty, use default stats
      let callerStatsObj = {
        total_callers: 0,
        active_callers: 0,
        inactive_callers: 0,
        assigned_callers: 0,
        unassigned_callers: 0
      };
      if (Array.isArray(callerStats) && callerStats.length > 0) {
        callerStatsObj = callerStats[0];
      }
